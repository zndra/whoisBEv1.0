...
def userFamilyUpdate(request):
    ...
    try:
        ...
        # Check if the user with the specified user_id exists and is not deleted
        userCursor.execute(
        ...
        userCursor.execute(
            'UPDATE "f_userFamily" SET "henBoloh" = %s, "ner" = %s, "dugaar" = %s WHERE "user_id" = %s AND "deldate" IS NULL',
            (henBoloh, ner, dugaar, user_id))
        myCon.commit()
        userCursor.close()
        disconnectDB(myCon)
