...
def userSocialUp(request):
    ...
    try:
        ...
        userCursor.execute(
            'SELECT id FROM "f_userSocial" WHERE "id" = %s', (id,))
        ...
        userCursor.execute('UPDATE "f_userSocial" SET "app" = %s, "site" = %s WHERE "id" = %s AND "user_id" = %s',(app, site, id,user_id))
        myCon.commit()
        userCursor.close()
        disconnectDB(myCon)
...
def userSocialDel(request):
    ...
    try:
        ...
        userCursor = myCon.cursor()
        userCursor.execute(
            'SELECT * FROM "f_userSocial" WHERE "id" = %s', (socialId,))
        user = userCursor.fetchone()
        if not user:
            response = {
                "responseCode": 555,
            ...
            socialcursor.execute('delete from "f_userSocial" WHERE "id" = %s', (socialId,))
            myCon.commit()
            socialcursor.close()
...
#             myCon = connectDB()
#             socialcursor = myCon.cursor()
#             socialcursor.execute('UPDATE "f_userSocial" SET "deldate" = %s WHERE "id" = %s AND "deldate" IS NULL',(date.today(), socialId,))
#             myCon.commit()
#             socialcursor.close()
